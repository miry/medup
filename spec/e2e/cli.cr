...
describe "CommandLine", tags: "e2e" do
  describe "options" do
    ...
    it "creates posts folder if missing" do
      ...
      actual = run_with ["-v0", "https://medium.com/notes-and-tips-in-full-stack-development/medup-backups-articles-8bf90179b094"]
    ...
    it "specifies another folder" do
      ...
      actual = run_with ["-v0", "-d", "tmp/posts", "https://medium.com/notes-and-tips-in-full-stack-development/medup-backups-articles-8bf90179b094"]
  ...
  describe "single post" do
    ...
    it "detects single not valid url" do
      actual = run_with ["-v0", "http://example.com"]
      actual[0].should contain(%{error: could not process http://example.com: unsupported content type text/html})
    end
    ...
    it "download medium from custom domain" do
      actual = run_with ["-v0", "https://jtway.co/git-minimum-for-effective-project-development-841a0b865ef0"]
    ...
    it "saves images in assets folder" do
      actual = run_with ["-v0", "--assets-images", "https://medium.com/notes-and-tips-in-full-stack-development/medup-backups-articles-8bf90179b094"]
      actual[0].should contain(%{Posts count: 1})
    ...
    it "update content for existing aritcles if option provided" do
      ...
      actual = run_with ["-v0", "--update", "https://medium.com/notes-and-tips-in-full-stack-development/medup-backups-articles-8bf90179b094"]
      actual[0].should contain(%{Create file ./posts/2020-09-16-medup-backups-articles.md})
      actual[1].should eq("")
  ...
  describe "publication" do
    ...
    it "download posts for publication" do
      actual = run_with ["-v0", "--publication", "notes-and-tips-in-full-stack-development"]
    ...
    it "unknown publication" do
      actual = run_with ["-v0", "--publication", "version"], false
      actual[0].should contain(%{GET /version/archive?format=json => 404 Not Found})
  ...
  describe "user" do
    ...
    it "download posts from user" do
      actual = run_with ["-v0", "--user", "kristinazakharchenko"]
    ...
    it "download posts from user without option" do
      actual = run_with ["-v0", "@kristinazakharchenko"]
      actual[0].should contain(%{GET /@kristinazakharchenko?format=json => 200 OK})
      actual[0].should contain(%{GET /_/api/users/a002e103d8f7/profile/stream?format=json&limit=100&source=overview => 200 OK})
      actual[1].should eq("")
    ...
    it "download posts from user recommendations" do
      actual = run_with ["-v0", "--user", "doctorow", "--recommended"]
